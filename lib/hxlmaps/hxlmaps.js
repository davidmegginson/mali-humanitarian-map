 ...
 */
hxlmaps.Map = function(mapId, mapConfig, onload) {
...
hxlmaps.Map.prototype.hideOSM = function () {
    this.osmLayer.removeFrom(this.map);
};

hxlmaps.Map.prototype.showOSM = function () {
    this.osmLayer.addTo(this.map);
};
    ...
    this.source.forEach(row => {
        ...
        for (var i = 0; i < row.values.length; i++) {
            if (row.columns[i]) {
                ...
                var includes = this.config.includePatternsParsed;
                var excludes = this.config.excludePatternsParsed;

                // check blacklist
                if (excludes.length > 0 && hxlmaps.matchTagPatternList(column, excludes)) {
                    continue;
                }

                // check whitelist
                if (includes.length > 0 && !hxlmaps.matchTagPatternList(column, includes)) {
                    continue;
                }
...
hxlmaps.Layer.prototype.makeAreaStyle = function (feature) {
    ...
    if (pcode) {
        ...
        if (row) {
            ...
            var value = count;
            if (this.config.colorMapType != "absolute") {
                value /= this.maxValue;
            }
            var color = hxlmaps.genColor(value, this.colorMap);
            return {
                stroke: true, // FIXME take from config
                weight: 1,
                color: "rgb(64, 64, 64)",
                fillColor: color
            };
...
hxlmaps.expandLayerConfig = function(config, source) {
    ...
    // Pre-parse whitelists and blacklists for properties to show in labels (if provided)
    if (config.includePatterns) {
        config.includePatternsParsed = config.includePatterns.map(hxl.classes.TagPattern.parse);
    } else {
        config.includePatternsParsed = [];
    }

    if (config.excludePatterns) {
        config.excludePatternsParsed = config.excludePatterns.map(hxl.classes.TagPattern.parse);
    } else {
        config.excludePatternsParsed = [];
    }
...
hxlmaps.guessAggregateColumnFromHXL = function(source, hashtag) {
    ...
    if (column) {
        ...
        if (hashtag) {
            console.error("No matching aggregation column", hashtag, source.columns);
        }
        return null;
    }
};
 ...
 * @param {object} colorMap - the colour map to process
 * @returns - a verbose color map for internal use
 */
hxlmaps.parseColorMap = function(colorMap) {
    ...
    return colorMap.sort((a, b) => a.value - b.value);
};
...
/**
 * Check if any tag pattern in a list matches a column.
 */
hxlmaps.matchTagPatternList = function(column, tagPatterns) {
    for (var i = 0; i < tagPatterns.length; i++) {
        if (tagPatterns[i].match(column)) {
            return true;
        }
    }
    return false;
 ...
 * @param value: a percentage value from 0.0 to 0.1
 ...
 */
hxlmaps.genColor = function(value, colorMap, alpha) {
    ...
    var rangeValue = (value - lower.value) / range;
    var valueLower = 1 - rangeValue;
    var valueUpper = rangeValue;
    var color = {
...
hxlmaps.controls.LegendControl = L.Control.extend({
    ...
    onAdd: function (map) {
        ...
        if (type == "areas") {
            ...
            var colorMap = this.options.layerConfig.colorMap;
            var minValue = 0;
            var maxValue = 1.0;
            if (this.options.layerConfig.colorMapType == "absolute") {
                minValue = colorMap[0].value;
                maxValue = colorMap.slice(-1)[0].value;
                this.options.min = minValue;
                this.options.max = maxValue;
            }
            var step = (maxValue - minValue) / 19.0;
            for (var value = minValue; value <= maxValue; value += step) {
                var color = hxlmaps.genColor(value, colorMap, alpha);
            ...
            var s = hxlmaps.numfmt(this.options.min);
            if (this.options.layerConfig.suffix) {
                s += this.options.layerConfig.suffix;
            }
            node.appendChild(hxlmaps.el('div', {class: 'min'}, s));

            s = hxlmaps.numfmt(this.options.max);
            if (this.options.layerConfig.suffix) {
                s += this.options.layerConfig.suffix;
            }
            node.appendChild(hxlmaps.el('div', {class: 'max'}, s));
        }
