    ...
    onAdd: function (map) {
        ...
        if (type == "areas") {
            ...
            if (this.options.layerConfig.colorMapType == "absolute") {
                this.options.min = colorMap[0].value;
                this.options.max = colorMap[2].value;
            }
            var step = (this.options.max - this.options.min) / 19.0;
            for (var value = this.options.min; value <= this.options.max; value += step) {
